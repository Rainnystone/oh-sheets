...
def compare_excel(gen_path, bench_path):
    """Compare two Excel files for exact data equality.

    Library function — returns True on match, False on mismatch, and
    raises on error (does not call sys.exit). Only the __main__ block
    translates the result/exception into a process exit code.

    Strings are trimmed; floats are rounded to 2 decimals; numeric
    columns are aligned to float64 to avoid int-vs-float false mismatches.
    """
    ...
    return bool(df_gen.equals(df_bench))
...
if __name__ == "__main__":
    ...
    try:
        match = compare_excel(args.generated, args.benchmark)
