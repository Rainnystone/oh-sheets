...
将数据提取到 Excel 时，经常会因为供应商稍微更改了发票格式而导致整个系统崩溃。`oh-sheets!` 作为智能协调器（Orchestrator）应运而生：
...
2. **约束契约与全局规则：** 对于每一个目标模板，都会生成一个严格的 `schema.json` 作为映射契约。同时，`rules.md` 会记录全局业务逻辑（例如：“总金额必须解析为浮点数”）。
